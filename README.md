...
UIは公式または第三者ミラーの画像、3Dモデル、音声、背景素材を読み込みません。キャラクターは公式日本語名、属性、数値とCSS生成の抽象トークンで識別し、検証対象を戦闘システムと操作フィードバックに限定します。画面はFluent 2を参考にしたフラットな中立面と青アクセントで統一し、画面文言は`ja-JP`言語資源から取得します。コスチュームの凸・潜在力・バーストを変えた場合、範囲・SP・CT表示も該当派生へ即座に切り替わります。
