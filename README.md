...
スキル潜在力は女神の涙を使う3ノードを個別にオン／オフできます。涙を使わない能力値ノードは強化済みで固定され、GUIを迂回した入力で無効化しても戦闘コアが拒否します。編成名を付けて保存すると、装備、凸、涙ノード、バースト上限、覚醒を含む検証済みシナリオが`data/scenarios/saved`へ保存され、そのJSONをGUIと`bd2-train`のどちらからでも読み込めます。
