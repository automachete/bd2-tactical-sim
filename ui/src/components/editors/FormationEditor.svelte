<script lang="ts">
  ...
  let { model, sideKey }: { model: BattleState; sideKey: SetupSide } = $props();

  let drag = $state<{ sideKey: SetupSide; index: number } | null>(null);
  let dropCell = $state<Cell | null>(null);
  let entries = $derived(model.partyUnits(sideKey));
  let grid = $derived(model.draft?.grid ?? { rows: 3, depths: 4, deployment_limit: 5, blocked: [] });
  ...
  const move = (row: number, depth: number): void => {
    const active = drag ?? model.editorFocus;
    if (active.sideKey === sideKey) model.moveDraftUnit(sideKey, active.index, row, depth);
    drag = null;
    dropCell = null;
  };
</script>
...
<section class="formation-side" id={sideKey === "enemy_units" ? "enemy-editor" : undefined} aria-labelledby={`${side.toLowerCase()}-formation-heading`}>
  <div class="formation-heading">
    <b id={`${side.toLowerCase()}-formation-heading`}>{sideKey === "player_units" ? t("preparation.player") : model.mode === "GOLDEN_COLOSSEUM" ? t("controller.golden") : t("preparation.enemyMcts")}</b>
    <button class="secondary-button" data-add-side={side} type="button" onclick={() => model.openPicker(side, side === "PLAYER" && model.draft?.mode === "MONSTER_CHASER" ? model.editorParty : 1)}>{t("preparation.addUnit")}</button>
  </div>
  <div
  ...
  >
    {#each cells() as cell (`${cell.row}-${cell.depth}`)}
      ...
      >
        {#if entry}
          {@const character = model.character(entry.unit.character_id)}
          <span
            class={`formation-token ${elementClass(character?.element)}`}
            class:selected={model.editorFocus.sideKey === sideKey && model.editorFocus.index === entry.index}
            ...
            onclick={(event) => { event.stopPropagation(); model.editorFocus = { sideKey, index: entry.index }; }}
            onkeydown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); model.editorFocus = { sideKey, index: entry.index }; } }}
            ondragstart={(event) => { drag = { sideKey, index: entry.index }; model.editorFocus = { sideKey, index: entry.index }; event.dataTransfer?.setData("text/plain", `${sideKey}:${entry.index}`); }}
            ondragend={() => { drag = null; dropCell = null; }}
          >
            <Avatar {character} />
            <span class="token-copy"><b>{character?.name ?? entry.unit.character_id}</b><small>{model.mode === "GOLDEN_COLOSSEUM" ? model.costume(entry.unit.costumes.find((item) => item.enabled !== false)?.costume_id ?? "")?.name ?? "" : t(`element.${character?.element ?? "NONE"}`)}</small></span>
          </span>
        {/if}
      </div>
    {/each}
  ...
  <div class="formation-roster" id={sideKey === "player_units" ? "player-roster" : "enemy-roster"} aria-label={t(sideKey === "player_units" ? "preparation.playerRosterAria" : "preparation.enemyRosterAria")}>
    {#each entries as entry (entry.index)}
      ...
      >
        <Avatar {character} />
        <span><b>{character?.name ?? entry.unit.character_id}</b>{#if model.mode === "GOLDEN_COLOSSEUM"}<small>{model.costume(entry.unit.costumes.find((item) => item.enabled !== false)?.costume_id ?? "")?.name ?? ""}</small>{/if}</span>
        <button type="button" class="roster-advanced" aria-label={t("formation.detailsAria", { name: character?.name ?? entry.unit.character_id })} onclick={(event) => { event.stopPropagation(); model.advancedEditor = { sideKey, index: entry.index }; }}>⚙</button>
        <button type="button" class="remove-unit" aria-label={t("formation.removeAria", { name: character?.name ?? entry.unit.character_id })} onclick={(event) => { event.stopPropagation(); model.removeDraftUnit(sideKey, entry.index); }}>×</button>
      </article>
    {/each}
  </div>
</section>
