<script lang="ts">
  ...
  let query = $derived(model.profileSearch.trim().toLocaleLowerCase("ja-JP"));
  let characters = $derived((model.catalog?.characters ?? []).filter((character) => (
    (model.profileElementFilter === "ALL" || model.profileElementFilter === character.element)
    && `${character.name} ${character.id}`.toLocaleLowerCase("ja-JP").includes(query)
  )));
  let character = $derived(model.selectedProfileId ? model.character(model.selectedProfileId) : undefined);
  let profile = $derived.by<CharacterProfile | null>(() => {
    if (!model.selectedProfileId) return null;
    return model.profileDrafts.get(model.selectedProfileId) ?? model.profileFor(model.selectedProfileId);
  });

  const change = (mutate: (profile: CharacterProfile) => void): void => {
    if (model.selectedProfileId) model.mutateProfile(model.selectedProfileId, mutate);
  };
  const changeEquipment = (equipment: Equipment): void => change((value) => { value.equipment = equipment; });
</script>

{#if model.dialog === "profiles"}
  <dialog use:modal class="sim-dialog profile-dialog" id="character-profile-dialog" data-testid="character-profile-dialog" onclose={() => model.close("profiles")}>
    <section class="dialog-frame profile-frame">
      <header class="dialog-title profile-title"><div><small>{t("profiles.caption")}</small><h1>{t("profiles.title")}</h1></div><button class="dialog-close" type="button" aria-label={t("profiles.closeAria")} onclick={() => model.close("profiles")}>×</button></header>
      <div class="profile-layout">
        <section class="profile-library" aria-labelledby="profile-library-heading">
          <header><div><h2 id="profile-library-heading">{t("profiles.libraryTitle")}</h2><p>{t("profiles.libraryHelp")}</p></div><strong id="profile-count">{characters.length} / {model.catalog?.characters.length ?? 0}</strong></header>
          <div class="profile-filters">
            <label class="profile-search"><span>{t("profiles.search")}</span><input id="profile-search" bind:value={model.profileSearch} type="search" autocomplete="off" placeholder={t("profiles.searchPlaceholder")} /></label>
            <div class="profile-element-filters" id="profile-element-filters" role="group" aria-label={t("profiles.elementFilterAria")}>
              {#each elements as element (element)}
                <button type="button" data-element={element} class:active={model.profileElementFilter === element} onclick={() => { model.profileElementFilter = element; }}>{t(element === "ALL" ? "profiles.allElements" : `element.${element}`)}</button>
              {/each}
            </div>
          </div>
            ...
            {:else}
              {#each characters as option (option.id)}
                ...
                >
                  ...
                  <em class="profile-card-status">{model.profileDirty(option.id) ? t("profiles.unsaved") : stored.is_default ? t("profiles.defaultBadge") : t("profiles.customBadge")}</em>
                </button>
                </div>
              {/each}
            {/if}
        ...
        <aside class="profile-detail" id="profile-detail" aria-live="polite">
          {#if character && profile && model.catalog}
            <header class={`profile-detail-header ${elementClass(character.element)}`}>
              ...
              <em id="profile-dirty" class:hidden={!model.profileDirty(character.id)}>{t("profiles.unsaved")}</em>
            </header>
            ...
            <EquipmentEditor catalog={model.catalog} characterId={character.id} equipment={profile.equipment} onchange={changeEquipment} />
            <footer class="profile-actions">
              <button type="button" class="secondary-button" data-testid="reset-character-profile" onclick={() => model.resetProfile(character.id)}>{t("profiles.reset")}</button>
              <button type="button" class="start-button profile-save" data-testid="save-character-profile" onclick={() => model.saveProfile(character.id)}>{t("profiles.save")}</button>
            </footer>
          {/if}
        </aside>
      </div>
